...
// Geometry Shader
[maxvertexcount(4)]     // Defines the upper bound on the number of vertices in the output
void main(point VertexOut gin[1], uint primID : SV_PrimitiveID, inout TriangleStream< GSOutput > output) {
	float2 TexC[4] = {
	...
	GSOutput gout;
	[unroll]        // Loop unwinding to increase speed at the expense of binary size (space-time tradeoff)
	for (int i = 0; i < 4; i++) {
		gout.pos = v[i];
		gout.tex = TexC[i];
